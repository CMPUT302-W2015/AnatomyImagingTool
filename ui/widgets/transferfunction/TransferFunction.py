...
# 		self.addPointAtCoord([1, 1], [1, 1, 1])
	...
	def removePointAtIndex(self, index):
		if len(self.points) > 2:
			self.colorFunction.RemovePoint(self.points[index].value)
			self.opacityFunction.RemovePoint(self.points[index].value)			
	...
	def updateTransferFunction(self):
		...
		for point in points:
			if index >= nrOfPoints:
				...
				self.opacityFunction.AddPoint(point.value, point.opacity)
			else:
				self.colorFunction.SetNodeValue(index, [point.value] + point.color + [0.5, 0.0])
				self.opacityFunction.SetNodeValue(index, [point.value, point.opacity, 0.5, 0.0])
			index += 1
